<?php

// Kiểm tra nếu chưa đăng nhập
if (!isset($_SESSION['nhanvien'])) {
    header('Location: login.php');
    exit();
}

// Lấy MaLoai từ session
$ma_loai = $_SESSION['ma_loai'];
?>
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    <div class="app-brand demo">
        <a href="index.php" class="app-brand-link">
            <span class="app-brand-logo demo me-1">
                <span style="color: var(--bs-primary)">
                    <img width="150" height="60" style="margin-top: 30px" src="../../assets/img/logo.png">
                </span>
            </span>
        </a>
    </div>
    <div class="menu-inner-shadow"></div>
    <ul class="menu-inner py-1">
        <!-- Dashboards -->
        <li class="menu-item">
            <a href="index.php" class="menu-link">
                <i class="menu-icon tf-icons ri-home-smile-line"></i>
                <div data-i18n="Dashboards">Dashboards</div>
            </a>
        </li>
        <!-- DỮ LIỆU -->
        
        <!-- Phân quyền menu -->
        <?php if ($ma_loai == '3'): // Nhân viên Kĩ Thuật ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">DỮ LIỆU</span></li>
            <li class="menu-item">
                <a href="station_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Bến Xe</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="bus_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Xe</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="typebus_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Loại Xe</div>
                </a>
            </li>
        <?php endif; ?>

        <?php if ($ma_loai == '2'): // Quản lý Tuyến Xe ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">QUẢN LÝ</span></li>
            <li class="menu-item">
                <a href="lichtrinh_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Lịch Trình</div>
                </a>
            </li>
            
        <?php endif; ?>

        <?php if ($ma_loai == '4'): // Nhân viên kế toán ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">QUẢN LÝ</span></li>
            <li class="menu-item">
                <a href="thongke.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Thống Kê Doanh Thu</div>
                </a>
            </li>
            </li><li class="menu-item">
                <a href="hoadon_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Thống Kê Hóa Đơn</div>
                </a>
            </li>
        <?php endif; ?>


        <?php if ($ma_loai == '1'): // Tài xế ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">QUẢN LÝ</span></li>
            <li class="menu-item">
                <a href="hanhkhach_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Hành Khách</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="list_chuyenxe_taixe.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Chuyến Xe Của Tài Xế</div>
                </a>
            </li>
        <?php endif; ?>


        <?php if ($ma_loai == '5'): // Nhân viên chăm sóc khách hàng ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">QUẢN LÝ</span></li>
            <li class="menu-item">
                <a href="hanhkhach_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Hành Khách</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="feedback_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Phản Hồi</div>
                </a>
            </li>
        <?php endif; ?>
        
        <?php if ($ma_loai == '0'): // Admin ?>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">DỮ LIỆU</span></li>
            <li class="menu-item">
                <a href="hanhkhach_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Hành Khách</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="user_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Nhân Viên</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="tuyenxe_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Tuyến Xe</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="bus_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Xe</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="typebus_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Loại Xe</div>
                </a>
            </li>
            <li class="menu-header fw-medium mt-4"><span class="menu-header-text">QUẢN LÝ</span></li>
            <li class="menu-item">
                <a href="lichtrinh_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Lịch Trình</div>
                </a>
            </li>
            <li class="menu-item">
                <a href="feedback_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Quản Lý Phản Hồi</div>
                </a>
            </li><li class="menu-item">
                <a href="thongke.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Thống Kê Doanh Thu</div>
                </a>
            </li>
            </li><li class="menu-item">
                <a href="hoadon_manager.php" class="menu-link">
                    <i class="menu-icon tf-icons ri-table-alt-line"></i>
                    <div data-i18n="Tables">Thống Kê Hóa Đơn</div>
                </a>
            </li>
            
        <?php endif; ?>
        
    </ul>
</aside>
